...
Dolphin and the replacement terminal-forward Kitty have now been reviewed with the corrected opaque vector underpainting. Native-size review establishes the current Hero delivery baseline: **SVG at 32/48/64/96/128/256/512 px, PNG at 1024 px**. At 512 px the current SVG reconstructions are visually close enough to the raster masters to prefer scalable delivery; at 1024 px the raster masters' additional material detail is clearly visible. This baseline remains reviewable for materially different future Hero constructions.
